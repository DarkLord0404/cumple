<x-app-layout>
    ...
    <div class="py-8" x-data="{ createOpen: {{ old('create_user_form') ? 'true' : 'false' }}, editing: null }" @open-create-user.window="createOpen = true">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section>
                <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h3 class="text-lg font-bold text-slate-900">Usuarios registrados</h3><p class="mt-1 text-sm text-slate-500">Administra datos, permisos y contraseñas desde cada fila.</p></div><p class="text-sm font-semibold text-slate-500">{{ $users->total() }} {{ $users->total() === 1 ? 'usuario' : 'usuarios' }}</p></div>
                <div class="mt-4 space-y-2">@forelse($users as $user)
                    <article class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-slate-200">
                        <div class="grid min-w-0 gap-2 p-3 sm:grid-cols-[minmax(0,1.5fr)_minmax(8rem,.8fr)_minmax(7rem,.65fr)_auto] sm:items-center sm:gap-4 sm:px-4">
                            <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><p class="truncate font-bold text-slate-900">{{ $user->name }}</p><span class="rounded-full px-2 py-0.5 text-[11px] font-semibold {{ $user->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">{{ $user->is_active ? 'Activo' : 'Inactivo' }}</span></div><p class="mt-0.5 truncate text-xs text-slate-500">{{ $user->email }}</p></div>
                            <div class="min-w-0 text-sm"><p class="text-[10px] font-bold uppercase tracking-wide text-slate-400 sm:hidden">Área</p><p class="truncate font-medium text-slate-700">{{ $user->area?->name ?? 'Sin área' }}</p></div>
                            <div class="text-sm"><p class="text-[10px] font-bold uppercase tracking-wide text-slate-400 sm:hidden">Rol</p><p class="font-medium text-slate-700">{{ $user->role_label }}</p></div>
                            <button type="button" @click="editing = editing === {{ $user->id }} ? null : {{ $user->id }}" class="mt-1 w-full shrink-0 rounded-lg bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-800 hover:bg-emerald-100 sm:mt-0 sm:w-auto" x-text="editing === {{ $user->id }} ? 'Cerrar' : 'Administrar'"></button>
                        </div>
